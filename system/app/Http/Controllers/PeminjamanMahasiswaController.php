...
class PeminjamanMahasiswaController extends Controller
{
    ...
    public function storepeminjaman_mahasiswa()
    {
        ...
        return redirect('peminjaman_mahasiswa')->with('success', 'Data Berhasil Ditambahkan');
    }

    public function editpeminjaman_mahasiswa($id)
    {
        $data['list_data_laboratorium'] = data_laboratorium::all();
        $data['data_peminjaman'] = data_peminjaman::find($id);
        $data ['mahasiswa']= auth()->guard('data_mahasiswa')->user();

        return view('mahasiswa/peminjaman.editpeminjaman_mahasiswa', $data);
     }

     public function showpeminjaman_mahasiswa($id)
     {
        $data['data_peminjaman'] =  data_peminjaman::find($id);
        $data ['mahasiswa']= auth()->guard('data_mahasiswa')->user();
         return view('mahasiswa/peminjaman.showpeminjaman', $data);
     }

     public function updatepeminjaman_mahasiswa(Request $request,$id)
     {
        ...
        return redirect('peminjaman_mahasiswa')->with('warning', 'Data Berhasil Diedit');
     }
}
